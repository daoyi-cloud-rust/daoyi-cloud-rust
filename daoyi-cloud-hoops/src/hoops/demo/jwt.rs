use anyhow::Result;
use daoyi_cloud_config::common_config::{self, JwtConfig};
...
pub fn get_token(uid: impl Into<String>) -> Result<(String, i64)> {
    let exp = OffsetDateTime::now_utc() + Duration::seconds(common_config::get().jwt.expiry);
    let claim = JwtClaims {
        uid: uid.into(),
    ...
    let token: String = jsonwebtoken::encode(
        ...
        &claim,
        &EncodingKey::from_secret(common_config::get().jwt.secret.as_bytes()),
    )?;
...
pub fn decode_token(token: &str) -> bool {
    ...
    decode::<JwtClaims>(
        token,
        &DecodingKey::from_secret(common_config::get().jwt.secret.as_bytes()),
        &validation,
    )
    .is_ok()
}
